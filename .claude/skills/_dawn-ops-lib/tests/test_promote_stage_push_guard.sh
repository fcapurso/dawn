...
out1=$(in_repo "$d" 'dawn::assert_stage_push_current staging_remote' 2>&1); rc1=$?
...
git -C "$d2" checkout -q staging
out2=$(in_repo "$d2" 'dawn::assert_stage_push_current staging_remote' 2>&1); rc2=$?
...
git -C "$d3" checkout -q staging
out3=$(in_repo "$d3" 'dawn::assert_stage_push_current staging_remote' 2>&1); rc3=$?
...
rc4=$(in_repo "$d4" 'dawn::assert_stage_push_current staging_remote' 2>&1); rc4_exit=$?
